...
NicheDocBot V2 now runs in three modes:

1. Local application mode for development.
2. Kubernetes Slack mode for real Slack interaction.
3. Kubernetes simulator mode for automated traffic generation and observability testing.

The simulator mode drives the same application endpoints used by Slack, but replaces the real Slack UI with a Kubernetes-based traffic generator. This allows the project to produce repeatable OpenTelemetry traces for GitHub search, repository ingestion, RAG retrieval, LLM calls, Slack-style outbound messages, and Q&A flows.

## Additional Documentation

- [Simulator Mode Runbook](README-simulator.md)
- [OpenTelemetry and Observability](README-observability.md)
